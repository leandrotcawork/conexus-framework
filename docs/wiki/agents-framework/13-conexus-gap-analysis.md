...
- **The single biggest cost bug** is that `core/agent_handler.py:69` appends current BRT timestamp into the system prompt on *every* call. Per [[05-context-engineering]] and [[12-cost-token-optimization]] this kills Anthropic-style prefix caching and (in practice) Gemini implicit caching, and adds zero value the model would not infer from a final user-message timestamp. Probably ~40–70 % input-token savings left on the table.
- **Tool schema generator silently downgrades unknown Python types to `{"type":"string"}`** (`core/tools/schema_gen.py:38`). `Literal`, `Enum`, `Annotated`, nested Pydantic models, and `dict[str,Any]` all become free-form strings — a correctness bug flagged directly by [[03-tool-integration]] (strict JSON Schema / `additionalProperties:false`) and [[12-cost-token-optimization]] (structured outputs halve retry spend).
...
| 10 | [[10-deployment-runtime]] | Fly `gru`, single machine, SQLite on volume | No `PRAGMA journal_mode=WAL` set on connect (relying on default/file state); APScheduler jobs not in `SQLAlchemyJobStore`; no `misfire_grace_time` / `coalesce` per job; no `/healthz`; ambiguous `ping_log` pending state | M | S | Explicit `PRAGMA journal_mode=WAL; synchronous=NORMAL` in `SqliteStore.__init__`; add misfire grace + coalesce; add `sent_at`/`failed_at`/`attempts` tri-state; `/healthz` HTTP endpoint |
...
[[05]] is blunt: Anthropic-style prompt caching needs a *stable prefix* marked with `cache_control`, and any per-call variability in that prefix (timestamps, user names, chat IDs) destroys the cache hit. `core/agent_handler.py:69` concatenates `cfg.system_prompt + f"\n\nData/hora atual (BRT): {now_brt}"` into the system message on every call. This is a three-line bug with massive cost implications: every reactive turn re-bills the full system prompt + tool schemas at full rate. Fix:
  1. System message = `cfg.system_prompt` verbatim, with `{"cache_control": {"type": "ephemeral"}}` on the last content block. Tool schemas get their own breakpoint. ([[12]] 4-breakpoint strategy.)
  2. Timestamp moves to a *trailing user message* (`<now>2026-04-15T14:30-03:00</now>`) right before the user body — post-cache, so it doesn't invalidate anything.
  3. `facts` and `chat_history` are currently concatenated into one bare string at `core/agent_handler.py:58-67`. Wrap in `<facts>…</facts>` and `<chat_history>…</chat_history>` per [[05]] delimiter guidance. Place `facts` *before* the cache breakpoint if they're stable per-user (they are) — free cache extension.
  4. `agents/ana/SKILL.md` system prompt + `main.py:225-268` appended "REGRA CRÍTICA" / "PROTOCOLO OBRIGATÓRIO DE PESQUISA" should be de-duplicated; currently each is string-appended every call and both contain overlapping content. Also, `main.py:263` has a UTF-8 encoding bug (`â€"` where an em-dash should be) — fix the source encoding.
...
The Fly-single-machine + SQLite-on-volume shape is *exactly* the [[10 §8]] recommendation. What's missing is operational hygiene:
  - **WAL not enforced.** `core/memory/sqlite_store.py` does not run `PRAGMA journal_mode=WAL; PRAGMA synchronous=NORMAL` on connect. [[10 §3]] marks this as mandatory for any concurrent-reader-under-async setup. Add a one-line `CREATE TABLE`-adjacent pragma at init.
  ...
  - **Ambiguous ping_log state.** `sent_at IS NULL` means "pending" OR "failed mid-body". `agents/pesquisador/jobs.py:35-37` prints to stderr and returns; the row stays NULL forever, and `ping_was_sent` returns False, so the next boot re-fires. That's at-least-once, acceptable for briefings (user gets a duplicate) but bad for `wiki_write`-touching jobs. Fix: `attempts` + `last_error` + `failed_at` columns; `ping_was_sent` returns True if `sent_at IS NOT NULL` *or* `attempts >= 3`.
...
- **Cache-busting from timestamp placement.** `core/agent_handler.py:74` (line updated as of Phase 6/7) — described §4.5. Every reactive turn re-bills the system prefix. Highest-ROI fix still pending.
...
- **`ping_log` idempotency tri-state.** §4.10. `sent_at IS NULL` ambiguous; `attempts` column missing; no `failed_at`.
